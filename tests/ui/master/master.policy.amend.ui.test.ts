...
test.beforeEach(async ({ logger }) => {
  logger.setLogTransport("amend_master_policy_ui_test");
  testData = new TestData(TestDataKeys.QUOTE_DATA_GATHER);
});


test('Verify user is able to amend the master policy @ui', async ({ page, request, createCustomerPage, classManagementPage, createNewQuotePage, customerPortfolioPage, logger, searchPage }) => {
  ...
  await test.step('Click on I WantTo button to proceed for Amend the Master Policy ', async () => {
    await customerPortfolioPage.doMasterPolicyAmend();
  });
});